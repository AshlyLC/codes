...
POWER_UP_INTERVAL = 5000  # 5 segundos
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT + 1:
            bullet._speed = 7  # Restablecer velocidad de la bala a su valor original
    ...
    if not game_over and not victory:
        ...
        if level == 1:
            ...
            if score >= 500 and level == 2:
                level = 3
                score = 0 
                enemy.increase_bullet_speed(6) 
                enemy.set_bullet_directions([0, 30, 70, 100, 150, 180, 250, 270, 290, 320, 350, 380])
                enemy.rect.x = SCREEN_WIDTH - 400
                enemy.rect.y = SCREEN_HEIGHT - 400
                enemy = Enemy() 
                all_sprites.add(enemy)
        ...
        if pygame.sprite.spritecollideany(ship, power_ups):
            for power_up in pygame.sprite.spritecollide(ship, power_ups, True):
                if power_up.type == 'speed':
                    bullet._speed = 20
                    pygame.time.set_timer(pygame.USEREVENT + 1, 5000)  # Temporizador de 5 segundos
